...
module.exports = function (ctx, size, options) {
    ...
    function drawLogo() {
        ...
        currentY += logoSize + scaled(conf.V_SPACING * 2) * 2;
    }

    /**
     * Draw organisation title
     *
     * @return void
     */
    function drawName() {
        var fontSize = scaled(30),
            x = size[0] / 4 * 3;

        ctx.font = fontSize + 'px ' + conf.BASE_FONT;
        ctx.fillStyle = colors.NAME;
        ctx.textAlign = 'center';
        ctx.fillText(organisation.name, x, currentY + fontSize / 2);
     ...
     */
    function drawTitle() {
        var fontSize = scaled(25),
            x = size[0] / 4 * 3;

        ctx.font = fontSize + 'px ' + conf.BASE_FONT;
        ctx.fillStyle = colors.TITLE;
        ctx.textAlign = 'center';
        ctx.fillText(position.title, x, currentY + fontSize / 2);
     ...
     */
    function drawShape() {
        var padding = scaled(conf.PADDING),
            shapeSize = size[1] - padding,
            origin = [ size[0] / 4, size[1] / 2 - padding / 2 ];
    ...
    function drawTags() {
        ...
        var workTags = [
            ...
            [ position.contract ? 'Contract' : 'Permanent', '#678d8c' ],
            [ position.remote ? 'Remote OK' : 'On-site', '#709c84' ]
            ];
